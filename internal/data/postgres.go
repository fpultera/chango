...
func (s *PostgresStorage) GetHistory(ctx context.Context, channelID string) ([]Message, error) {
	query := `SELECT id, content, channel_id, created_at FROM messages 
              WHERE channel_id = $1 AND is_private = false ORDER BY created_at ASC LIMIT 50`
	...
	for rows.Next() {
		var m Message
		rows.Scan(&m.ID, &m.Content, &m.ChannelID, &m.CreatedAt)
...
func (s *PostgresStorage) CreateUser(ctx context.Context, username, password string) error {
	...
	_, err := s.Pool.Exec(ctx, "INSERT INTO users (username, password) VALUES ($1, $2)", username, string(hashed))
	return err
}

func (s *PostgresStorage) GetUserByUsername(ctx context.Context, username string) (*User, error) {
	var u User
	err := s.Pool.QueryRow(ctx, "SELECT id, username, password FROM users WHERE username = $1", username).Scan(&u.ID, &u.Username, &u.Password)
	if err != nil { return nil, err }
	return &u, nil
}